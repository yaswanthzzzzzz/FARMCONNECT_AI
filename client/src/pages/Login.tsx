...
function workspacePath(role: string, fallback: string) {
  return role === "buyer" ? "/buyer" : role === "farmer" ? "/farmer" : fallback;
}
...
export default function Login() {
  ...
  useEffect(() => {
    if (!loading && user && user.role !== "user") navigate(workspacePath(user.role, returnTo));
  ...
  const login = trpc.auth.login.useMutation({
    onSuccess: data => {
      ...
      navigate(workspacePath(data.role, "/login"));
    },
    onError: error => setCredentialError(error.message || "Invalid username or password."),
  });
  ...
  return <div className="container grid min-h-[calc(100vh-72px)] items-center gap-8 py-10 sm:py-16 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16"><div className="max-w-xl"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-[15px] bg-[#1b5e3c] text-[#e4f3cc] shadow-[0_8px_18px_rgba(27,94,60,0.2)]"><Leaf size={22} /></span><div><div className="text-sm font-bold text-[#18332a]">FarmConnect <span className="text-[#6b8f60]">AI</span></div><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#8b9c8e]">Smarter paths to market</div></div></div><div className="mt-10 text-xs font-bold uppercase tracking-[0.16em] text-[#6b8f60]">Secure workspace access</div><h1 className="mt-3 max-w-lg text-4xl font-bold tracking-[-0.06em] text-[#18332a] sm:text-5xl">Choose a smarter path to market.</h1><p className="mt-5 max-w-lg text-base leading-7 text-[#5f7465]">Connect your FarmConnect workspace to the selling or sourcing decisions that matter — with clear ownership, transparent factors and no black-box handoffs.</p><div className="mt-8 grid gap-3 text-sm text-[#496454] sm:grid-cols-2"><div className="flex items-start gap-3"><CheckCircle2 size={18} className="mt-0.5 shrink-0 text-[#6f9b63]" />Your listings and requirements stay tied to your account.</div><div className="flex items-start gap-3"><ShieldCheck size={18} className="mt-0.5 shrink-0 text-[#6f9b63]" />Secure session handling through the configured OAuth provider.</div></div></div><Surface className="mx-auto w-full max-w-md p-7 sm:p-9"><div className="text-xs font-bold uppercase tracking-[0.15em] text-[#6b8f60]">Continue securely</div><h2 className="mt-3 text-2xl font-bold tracking-[-0.04em] text-[#18332a]">Enter your workspace</h2><p className="mt-2 text-sm leading-6 text-[#718274]">Use Google or your FarmConnect username. Passwords are protected and never shared with the OAuth provider.</p>{(error || credentialError) && <div className="mt-6"><AlertBanner tone="warning">{credentialError || "Authentication could not be completed. Please try again."}</AlertBanner></div>}<button type="button" onClick={() => startLogin(returnTo)} className="mt-7 flex w-full items-center justify-center gap-3 rounded-2xl bg-[#1b5e3c] px-5 py-3.5 text-sm font-bold text-white shadow-[0_10px_24px_rgba(27,94,60,0.18)] transition hover:bg-[#164d31] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#cfe8b2]"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-white text-xs font-bold text-[#1b5e3c]">G</span>Continue with Google <ArrowRight size={16} /></button><div className="my-6 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.16em] text-[#a0aca1]"><span className="h-px flex-1 bg-[#e3ebe0]" />or<span className="h-px flex-1 bg-[#e3ebe0]" /></div><form onSubmit={submitLogin} className="space-y-4"><label className="block text-sm font-semibold text-[#496454]">Username<input required autoComplete="username" value={username} onChange={event => setUsername(event.target.value)} className="mt-2 w-full rounded-xl border border-[#dfe9d9] bg-white px-3.5 py-3 text-[#18332a] outline-none focus:border-[#6f9b63] focus:ring-4 focus:ring-[#eff8e9]" /></label><label className="block text-sm font-semibold text-[#496454]">Password<input required type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-[#dfe9d9] bg-white px-3.5 py-3 text-[#18332a] outline-none focus:border-[#6f9b63] focus:ring-4 focus:ring-[#eff8e9]" /></label><button type="submit" disabled={login.isPending} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-[#1b5e3c] px-5 py-3.5 text-sm font-bold text-[#1b5e3c] transition hover:bg-[#eff8e9] disabled:opacity-60">{login.isPending && <Loader2 size={16} className="animate-spin" />}Sign In</button></form><Link href="/register" className="mt-4 block text-center text-sm font-bold text-[#1b5e3c] hover:underline">Create Account <ArrowRight size={15} className="ml-1 inline" /></Link><p className="mt-5 text-center text-xs leading-5 text-[#819086]">Returning users go directly to their saved workspace. New users choose a role after their first sign-in.</p><Link href="/demo" className="mt-6 block rounded-2xl border border-[#d9c99d] bg-[#fff9ea] p-4 text-center transition hover:border-[#b9a76d] hover:bg-[#fff4d5]"><span className="block text-sm font-bold text-[#856117]">Explore Demo <ArrowRight size={15} className="ml-1 inline" /></span><span className="mt-1 block text-xs leading-5 text-[#8b7746]">Explore FarmConnect with sample data. No account required.</span></Link>{user && <button type="button" onClick={() => void logout()} className="mt-6 inline-flex items-center gap-2 text-xs font-semibold text-[#718274] hover:text-[#1b5e3c]"><LogOut size={14} /> Sign out</button>}</Surface></div>;
}
