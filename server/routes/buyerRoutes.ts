...
export function registerBuyerRoutes(app: Express) {
  app.get("/api/buyers/requirements", async (req, res, next) => {
    try {
      res.json({ requirements: await buyerService.listRequirements(typeof req.query.buyerKey === "string" ? req.query.buyerKey : undefined) });
  ...
  app.post("/api/buyers/requirements", async (req: Request, res: Response, next) => {
    try {
      ...
      res.status(201).json({ requirement: await buyerService.create(parsed.data) });
    } catch (error) { next(error); }
  });
  ...
  app.get("/api/buyers/requirements/:id/matches", async (req, res, next) => {
    try {
      ...
      res.json(await buyerService.calculate(id, typeof req.query.buyerKey === "string" ? req.query.buyerKey : undefined));
    } catch (error) {
